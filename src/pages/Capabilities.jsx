          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {capabilitiesContent.items.map((capability, index) => (
              <motion.div
                ...
                className="bg-beige p-8 border border-border-line hover:border-accent hover:shadow-lg transition-all"
              >
                <h3 className="text-xl font-bold text-text-on-light mb-3">{capability.title}</h3>
                <p className="text-text-on-light/70">{capability.description}</p>
              </motion.div>
            ))}
          </div>
        </Container>
